...
class production_department_keypi(models.Model):
    _name = "production_department.keypi"
    ...
    def create(self, vals):
        if vals.get('id', ('New')) == ('New'):
            vals['id'] = self.env['ir.sequence'].next_by_code('production_department_keypi.id') or _('New')
        res = super(production_department_keypi, self).create(vals)
        return res
